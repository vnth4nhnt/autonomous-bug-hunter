...
- Default ordinary testing to <=5 requests/second per host and <=10 active request workers globally. After stable calibration, the pinned unauthenticated GET-only sensitive baseline may use <=25 requests/second per host and <=40 workers globally; at `ffuf -t 20`, run at most two instances. Count every tool and job toward the applicable total, and obey any lower program limit. Back off immediately on 429, WAF bursts, latency/error growth, instability, or operational impact. Race tests use <=20 controlled requests unless the program authorizes more.
...
Before a new traffic batch emit: `SCOPE | QUEUE | HOST/SURFACE | OBJECTIVE/HYPOTHESIS | STRATEGY/WHY | CONTROLS | EXPECTED EVIDENCE | STOP/PIVOT`. Routine continuation reuses the checkpoint.

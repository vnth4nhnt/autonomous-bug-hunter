...
2. **Host recon:** use `recon-pipeline`. Passive discovery feeds the host/service queue. Every verified web host gets DNS/TLS/HTTP verification, the pinned sensitive baseline, a representative deployment observation and the strongest evidence roots. Route every unresolved value edge, including redirects and external consumers. Refresh passive discovery during work and before closure.
...
WSTG is the mandatory coverage spine, not a blind payload ritual. Maintain a complete versioned test-case ledger: every exact WSTG v4.2 scenario and current official delta maps to its owning surfaces or positive `not-applicable` evidence, and every mapped case receives decision-specific terminal evidence across material axes. One category label, 403, payload, scanner result, or an LLM's memory is not coverage.
...
- Resolve `engagements/<name>/` as the only target-output root and `knowledge-base/` as its evidence root. From the first command, keep every target input, output, capture, download and retained scratch there; drafts use the reporting paths below. Never place target data in the project root, configuration, tool/runtime or another engagement. `.runtime/` holds reusable tools and bounded MCP spill only. Ingest target-specific spill after each browser batch and clear it before more traffic or handoff. Keep secret state isolated and tool-managed.
- Under `engagements/<name>/knowledge-base/`, create only needed entries: `scope/`; `working-state.md`; `target-profile.md`; `surface-inventory.md`; `surfaces.md`; `identities.md`; append-only `tests/wstg-coverage.md`, `tests/<surface-or-lane>.md`, `candidates/<id>.md`, `validations/<id>.md`, `audits/<id>.md`; and redacted `artifacts/`.
...
- `surface-inventory.md` is the current human-readable operation ledger with exactly `Domain | Endpoint / interaction | Method / action | Params | Body / message | Authentication | Authorization`. Give every distinct HTTP method, RPC/GraphQL operation, message/event, security-bearing UI action and material request-shape or authentication/authorization variant its own row, even when the path is shared. Normalize parameter locations and body/message field shapes with placeholders; use `[-]` for absent, `[+] <mechanism>` for required authentication and `[?] unresolved` for unknown authentication facts. Authorization uses only `PUBLIC`, `AUTHENTICATED`, `SELF`, `OWNER`, `MEMBER`, `ROLE:<role>`, `CAPABILITY`, `SERVICE` or `UNKNOWN`; join simultaneous requirements with `+`. Never use vague labels such as `normal user`, `authorized user` or `privileged user`. Never store example secrets or live personal values. Keep detailed policy, provenance, knowledge state and canonical decision mapping in `surfaces.md`; never omit a discovered interaction merely because a column is unresolved.
...
- Each evidence reference records the method fingerprint, capture time, scoped asset, role/fixture label, tool/version/material options, redaction, artifact/request id, and SHA-256 of every redacted file.
...
Skills define methods; agents provide isolated execution. The orchestrator is the sole scheduler and writer of shared queues, `target-profile.md`, `surface-inventory.md`, `surfaces.md`, aggregate candidate state, and reports.
...
2. Every verified web host completed the exact-input sensitive-path/metadata baseline and evidence-root collection.

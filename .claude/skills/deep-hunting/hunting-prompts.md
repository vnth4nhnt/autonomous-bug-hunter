...
Use [OWASP WSTG v4.2](https://owasp.org/www-project-web-security-testing-guide/v42/) as the reproducible test-case base. At engagement start, derive a complete catalog from its versioned index and record every scenario as `WSTG-v42-<category>-<number>`, title and versioned URL in the WSTG coverage lane, with source digest and access date. Map each case to one or more canonical surfaces or give positive `not-applicable` evidence; category-level labels, an LLM's memory and omitted ids are not coverage. Inspect the [official WSTG project index](https://owasp.org/www-project-web-security-testing-guide/) for material stable/development deltas and keep them in a separate exact-page/date delta ledger; never present a development id as stable or let it replace the v4.2 baseline. Use the current official [OWASP ASVS](https://owasp.org/www-project-application-security-verification-standard/) stable release (5.0.0 at this method revision) and current mechanism standards as control-objective sources, not as claims that a black-box test can verify every requirement. For OAuth/OIDC decisions include [RFC 9700](https://www.rfc-editor.org/rfc/rfc9700.html) and the applicable OpenID Connect specification; use the actual protocol/profile implemented. Do not invent ids when the versioned source is available.
...
Maintain the catalog ledger throughout discovery. New surfaces, roles, states, channels or mechanisms reopen applicability for affected cases. A test may close multiple WSTG cases only when its evidence and controls satisfy every named objective; one case may require separate rows across materially different surfaces or axes. Surface closure requires terminal results for all mapped cases, while engagement closure requires every catalog and delta entry to have an evidence-backed disposition.

...
2. Reproduce once from fresh state with independent fixtures and markers. Repeat in proportion to instability, timing, statefulness, and impact; deterministic evidence does not require ritual duplicate runs, while noisy claims require enough controlled samples.
...
6. If all material premises survive, follow only the controlled severity-changing rungs needed to bound concrete impact. Apply the relevant proof floors; never inherit impact from an unexecuted chain step.
...
Use fresh tool state and fixtures independent of the hunter. Reproduce wire-level claims with the smallest tool-managed protocol client that preserves material semantics; reproduce browser-context claims in a fresh headless `playwright` context; use `playwright-headed` only for an unavoidable operator interaction or a recorded headless limitation, then return to headless. Use a fresh `interactsh-client` correlation for outbound claims. Prefer a second observation path when it does not change the parser or state being tested. Recalculate timing/statistical claims from preserved raw samples, not the hunter's summary.
...
If refutation reveals a new scoped host/service, return exact recon work. If it reveals new JavaScript, operation, subject, state, parser, consumer, identifier transport, implementation, or channel, return exact extraction work. A terminal candidate disposition never closes another test or its canonical surface.

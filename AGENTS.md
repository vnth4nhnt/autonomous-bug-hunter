...
Run the kernel's five queues through their terminal invariants. Keep `surface-inventory.md` as the interaction projection, `surfaces.md` as the canonical security-decision graph and HUNT source, and the other current-state records synchronized by exact ids. Audit and reporting are read-only consumers; commit every auditor `REOPEN` to its owning queue and continue. Pause only at the kernel's exact `WAITING-ON-OPERATOR` condition.
